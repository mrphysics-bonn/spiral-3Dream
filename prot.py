...
def create_hdr(hdr, params):
    ...
    dtime.value_ = params["dwelltime"] * 1e6 
    tdel = ismrmrd.xsd.userParameterDoubleType()
    tdel.name = 'traj_delay'
    tdel.value_ = params["traj_delay"] # this field can be used for any additional trajectory delay [s]
    nseg = ismrmrd.xsd.userParameterDoubleType()
    nseg.name = 'nsegments'
    nseg.value_ = params["nsegments"] # number of ADC segments
    tmin = ismrmrd.xsd.userParameterDoubleType()
    tmin.name = 't_min'
    tmin.value_ = params["t_min"] # starting time of t-vector for B0-correction

    hdr.userParameters = ismrmrd.xsd.userParameters()
    hdr.userParameters.append(dtime)
    hdr.userParameters.append(tdel)
    hdr.userParameters.append(nseg)
    hdr.userParameters.append(tmin)
