...
def gre_3DrefscanLinear(seq, prot=None, system=Opts(), params=None):
    ...
    print(calc_duration(gz_pre0,gx_pre))
    ...
    min_TR = calc_duration(gz_pre0,gx_pre) + calc_duration(gz) + calc_duration(gx) + delay_TE + max_dur
        ...
        for j in range(np.size(gz_area)):
            ...
            for i in range(np.size(gy_area)):
                ...
                if prot is not None:
                    ...
                    if j == np.size(gz_area)-1 and i == np.size(gy_area)-1:
