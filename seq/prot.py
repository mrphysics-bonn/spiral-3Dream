...
def create_hdr(hdr, params):
    ...
    if "traj_delay" not in params:
        params["traj_delay"] = 0
    if "t_min_fid" not in params:
        params["t_min_fid"] = 0
    if "t_min_ste" not in params:
        params["t_min_ste"] = 0
    if "redfac1" not in params:
        params['redfac1'] = 1
    if "redfac2" not in params:
        params['redfac2'] = 1
    ...
    t_min_fid = ismrmrd.xsd.userParameterDoubleType()
    t_min_fid.name = 't_min_fid'
    t_min_fid.value = params["t_min_fid"] # starting time of t-vector for B0-correction
    t_min_ste = ismrmrd.xsd.userParameterDoubleType()
    t_min_ste.name = 't_min_ste'
    t_min_ste.value = params["t_min_ste"] # starting time of t-vector for B0-correction
    os_factor = ismrmrd.xsd.userParameterDoubleType()
    os_factor.name = 'os_factor'
    os_factor.value = params["os_factor"] # starting time of t-vector for B0-correction
    ...
    hdr.userParameters.userParameterDouble.append(t_min_fid)
    hdr.userParameters.userParameterDouble.append(t_min_ste)
    ...
    limits.kspace_encoding_step_2.maximum = params["npartitions"] - 1
    limits.kspace_encoding_step_2.center = 0
    ...
    acc_fac.kspace_encoding_step_1 = params['redfac1']
    acc_fac.kspace_encoding_step_2 = params['redfac2'] # use this field for multiband factor or Rz
