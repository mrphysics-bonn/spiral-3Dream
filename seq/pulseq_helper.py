...
def trap_from_area(area, system, slewrate = None):
    """
...
def check_resonances(grads):
    ...
    for key,grad in enumerate(grads):
        ...
        if (500 <= freq_max[key] <= 600 or 930 <= freq_max[key] <= 1280):
            raise ValueError('Frequency peak {:.0f} Hz of axis {} is in the forbidden range of 500-600Hz or 930-1280 Hz.'.format(freq_max[key],key))
