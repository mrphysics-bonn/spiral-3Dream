...
# -*- coding: utf-8 -*-
"""
3DREAM cartesian version (parameters from threedream by P.Ehses) for all three timing schemes (but with Centric two phase encoding order)
"""
...
# General
seq_dest = "scanner"    # save parameter: 'None': save nothing, 'scanner': save seq & prot in seq/prot folder & in exchange folder -> muss noch unten definiert werden 'sim': save seq in sim folder
seq_name = '3DREAM_cart_centric'    # sequence/protocol filename
...
# Preperation scans
prepscans = 2        # number of dummy preparation scans (bei STEAM notwendig?-> ja, für fid in steady state!)
...
# Phasenkodierung:
# first phase encoding direction (Gz)
phase1_comp,phase1_areas = pe.CentricOrder(N=Nz, fov=fov, comp=True)  #[1/m]
# second phase encoding direkction (Gy)
phase2_comp,phase2_areas = pe.CentricOrder(N=Ny, fov=fov, comp=True) #[1/m]
...
for d in range(prepscans):
    ...
    gz_pre = make_trapezoid(channel='z', area=phase1_areas[0], duration=calc_duration(gm1), system=system)
    gy_pre = make_trapezoid(channel='y', area=phase2_areas[0], duration=calc_duration(gm1), system=system) # hat dieselbe dauer wie gx_pre       
...
# imaging scans
for j in range(np.size(phase1_areas)):
    # first phase encoding
    gz_pre = make_trapezoid(channel='z', area=phase1_areas[j], duration=calc_duration(gm1), system=system)
    
    for i in range(np.size(phase2_areas)):
        #rf spoiling
        rf.phase_offset = rf_phase / 180 * np.pi
        adc_ste.phase_offset = rf_phase / 180 * np.pi
        adc_fid.phase_offset = rf_phase / 180 * np.pi
        ...
        # phasenkodierung
        seq.add_block(rf, rf_delay)
        gy_pre = make_trapezoid(channel='y', area=phase2_areas[i], duration=calc_duration(gm1), system=system) # hat dieselbe dauer wie gx_pre       
        seq.add_block(gm1, gz_pre, gy_pre)
        seq.add_block(te_ste_delay)
        ...
        for n in range(2):
            if prot is not None:
                ...
                acq.idx.kspace_encode_step_1 = int(Ny/2+phase1_comp[i])    # kspace counter (k-raum schritte in ky)
                acq.idx.kspace_encode_step_2 = int(Nz/2+phase2_comp[j]) # only 2D atm -> zählt die k-raum schritte in kz bei der 3D-Bildgebung
                acq.idx.slice = 0
                ...
                if i == np.size(phase2_areas)-1 and j == np.size(phase1_areas)-1:
